  ...
  var _ref = _asyncToGenerator(function* (req, res) {
    ...
    try {
      ...
      if (result && result.length > 0) {
        ...
        yield conn.query("UPDATE header_config SET routes=?, buttons=?, logo_img_path=IFNULL(?, logo_img_path) WHERE id=1", [JSON.stringify(routes), JSON.stringify(buttons), logoImgPath]);
      } else {
        console.log("Inserting new record");
        yield conn.query("INSERT INTO header_config (id, routes, buttons, logo_img_path) VALUES (1, ?, ?, ?)", [JSON.stringify(routes), JSON.stringify(buttons), logoImgPath]);
      }
      res.json({
  ...
  var _ref2 = _asyncToGenerator(function* (req, res) {
    try {
      ...
      if (results.length > 0) {
        ...
        var buttons = JSON.parse(config.buttons);
        res.json({
          id: config.id,
          routes: routes,
          buttons: buttons,
